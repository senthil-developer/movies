            ...
            return(
                <div className='flex flex-col h-auto gap-1' key={cast.id}>
                    ...
                    <div className='flex items-center relative w-[150px] h-[150px] mx-2 rounded-full'> 
                        <Image src={cast.profile_path ? `https://image.tmdb.org/t/p/original${cast.profile_path}`: "/default.png"} width={150} height={150} placeholder='blur' blurDataURL='/default.png' style={{objectFit:'cover',width:'150px',height:'150px'}} alt={cast.name} className='rounded-full object-top'/>
                    </div>
                    </Link>
                    <Link href={{pathname: `/person/${cast.id}`, query: { name: `${cast.name}` },}} className='text-xs font-bold'> 
